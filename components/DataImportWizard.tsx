...
export const DataImportWizard: React.FC<Props> = ({
  type,
  onImport,
  onClose,
}) => {
      ...
      mappings.forEach((m) => {
        ...
        if (m.targetField === "gallons" && Number(val) <= 0) {
          errors.push({
            row: idx + 1,
            field: m.targetField,
            message: "Gallons must be positive",
          });
        }
        ...
        <div className="flex-1 overflow-auto p-10">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {targetFields.map((field) => (
                      <tr key={field}>
                        <td className="px-8 py-6 font-black text-white uppercase tracking-tighter text-xs">
                          {field}
                        </td>
          ...
          {step === 3 && dryRun && (
            <div className="space-y-8 animate-in slide-in-from-right-10 duration-500">
              {/* RESULTS BANNER */}
              <div
                className={`p-8 rounded-[2.5rem] border ${dryRun.success ? "bg-emerald-500/5 border-emerald-500/10" : "bg-red-500/5 border-red-500/10"} flex items-center justify-between`}
              >
                <div className="flex items-center gap-6">
                  <div
                    className={`p-5 rounded-full ${dryRun.success ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}
                  >
                    {dryRun.success ? (
                  ...
                  <div>
                    <h3
                      className={`text-2xl font-black uppercase tracking-tighter ${dryRun.success ? "text-emerald-500" : "text-red-500"}`}
                    >
                      {dryRun.success
                    ...
                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">
                      Processed {dryRun.totalRows} records • {dryRun.validRows}{" "}
                      valid • {dryRun.errorRows} errors
                    </p>
                  </div>
                </div>
                {!dryRun.success && (
                  <div className="text-right">
                    <div className="text-xs font-black text-red-500 uppercase font-mono">
                      {dryRun.errorRows} CRITICAL ERRORS
                    </div>
                    <button className="text-[10px] font-black text-slate-500 uppercase hover:underline mt-2">
                      View Error Manifest
                    </button>
                  </div>
                )}
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">
                      ...
                      {dryRun.preview.map((row, i) => (
                        <tr key={i}>
                          {targetFields.slice(0, 5).map((f) => (
                            <td
                              key={f}
                              className="px-6 py-4 text-[10px] font-mono text-slate-300"
                            >
                              {String(row[f])}
                            </td>
                          ))}
        ...
        <div className="p-10 border-t border-white/5 bg-slate-900/20 shrink-0 flex justify-between items-center">
          ...
          <div className="flex gap-4">
            ...
            {step === 2 && (
              <button
                onClick={runDryRun}
                className="px-12 py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-3xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-blue-500/20 flex items-center gap-3 font-inter disabled:opacity-60 disabled:cursor-not-allowed"
