        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl flex flex-col overflow-hidden shadow-2xl h-[400px]">
            ...
            <div className="bg-slate-950 px-6 py-4 border-b border-slate-800 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-4">
                    <h3 className="text-sm font-black text-white uppercase tracking-widest">Operational Sequence</h3>
                    <div className="flex gap-2">
                        <span className="bg-blue-600/10 text-blue-500 border border-blue-500/20 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter">Active: {loads.filter(l => l.status === LOAD_STATUS.Active).length}</span>
                        <span className="bg-slate-800 text-slate-400 border border-slate-700 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter">Total: {loads.length}</span>
                    </div>
                </div>
                <div className="flex gap-8 text-[9px] font-black text-slate-500 uppercase tracking-widest">
                    <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-slate-800" /> Planned</div>
                    <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-600 shadow-[0_0_8px_rgba(59,130,246,0.5)]" /> Execution</div>
                    <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-600" /> Completed</div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-x-auto overflow-y-auto no-scrollbar">
                <div className="min-w-[1000px]">
                    {sortedLoads.map((load, idx) => (
                        <div key={load.id} className={`flex items-center border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors group ${idx % 2 === 0 ? 'bg-slate-900/30' : ''}`}>
                            ...
                            <div className="flex-1 px-8 relative h-16 flex items-center">
                                ...
                                <div className="relative flex-1 flex items-center h-full">
                                    ...
                                    <div className="w-full flex items-center gap-0">
                                        <div className={`h-1.5 rounded-l-full relative transition-all duration-700 ${load.status === LOAD_STATUS.Active || load.status === LOAD_STATUS.Delivered ? 'bg-blue-600 w-1/3' : 'bg-slate-800 w-1/12'}`}>
                                            <div className="absolute -top-6 left-0 text-[7px] font-black text-slate-500 whitespace-nowrap">PICKUP</div>
                                            {(load.status === LOAD_STATUS.Active || load.status === LOAD_STATUS.Delivered) && (
                                        ...
                                        <div className={`h-1.5 relative transition-all duration-1000 ${load.status === LOAD_STATUS.Active ? 'bg-gradient-to-r from-blue-600 to-blue-400 w-1/2 animate-pulse' : load.status === LOAD_STATUS.Delivered ? 'bg-blue-400 w-1/2' : 'bg-slate-800 w-1/12'}`}>
                                            <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[7px] font-black text-slate-500 whitespace-nowrap">TRANSIT</div>
                                        ...
                                        <div className={`h-1.5 rounded-r-full relative transition-all duration-700 ${load.status === LOAD_STATUS.Delivered ? 'bg-green-600 flex-1' : 'bg-slate-800 flex-1'}`}>
                                            <div className="absolute -top-6 right-0 text-[7px] font-black text-slate-500 whitespace-nowrap text-right">DELIVERY</div>
