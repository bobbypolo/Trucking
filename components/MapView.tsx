...
export const MapView: React.FC<Props> = ({ loads }) => {
    // Generate random mock positions for loads for demonstration
    // In a real app, these would come from driver GPS coordinates
    const loadPositions = useMemo(() => {
        return loads.filter(l => l.status !== LOAD_STATUS.Delivered && l.status !== LOAD_STATUS.Cancelled).map(l => ({
            load: l,
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case LOAD_STATUS.Active: return 'text-blue-500';
            case LOAD_STATUS.Booked: return 'text-green-500';
            case LOAD_STATUS.Planned: return 'text-slate-500';
    ...
    return (
        <div className="relative w-full h-[500px] bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl group">
            ...
            {/* Mock Map Contours (SVG) */}
            <svg className="absolute inset-0 w-full h-full opacity-20 pointer-events-none" viewBox="0 0 1000 500">
                <path d="M100,200 Q300,50 500,250 T900,100" fill="none" stroke="#1e293b" strokeWidth="2" />
                <path d="M50,400 Q250,300 450,450 T850,350" fill="none" stroke="#1e293b" strokeWidth="2" />
                <path d="M400,0 Q600,200 400,400" fill="none" stroke="#1e293b" strokeWidth="2" />
            </svg>

            {/* Load Markers */}
            <div className="absolute inset-0">
                {loadPositions.map((pos, idx) => (
                    <div
                        key={pos.load.id}
                    ...
                    >
                        ...
                        <div className={`relative p-2 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl flex items-center justify-center ${getStatusColor(pos.load.status)}`}>
                            {pos.load.status === LOAD_STATUS.Active ? (
                                <Navigation className="w-4 h-4" style={{ transform: `rotate(${pos.heading}deg)` }} />
                            ) : (
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 bg-slate-900 border border-slate-700 rounded-xl p-3 shadow-2xl opacity-0 group-hover/marker:opacity-100 transition-opacity z-50 pointer-events-none">
                                <div className="text-[10px] font-black uppercase text-slate-500 mb-1">Load #{pos.load.loadNumber}</div>
                                <div className="text-xs font-bold text-white mb-2">{pos.load.commodity || 'General Freight'}</div>
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2 text-[8px] text-slate-400 font-bold uppercase">
                                        <Clock className="w-2.5 h-2.5" /> Checked In: 08:45 AM
                                    </div>
                                ...
                                <div className="mt-2 pt-2 border-t border-slate-800 flex justify-between items-center text-[7px] font-black text-slate-500">
                                    <span>LAT: {pos.y.toFixed(2)}</span>
                                    <span>LNG: {pos.x.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2">
                <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-lg p-1 shadow-lg flex flex-col items-center">
                    <button className="p-2 hover:bg-slate-800 text-slate-400 rounded-md">+</button>
            ...
            <div className="absolute bottom-4 right-4 w-64 bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-xl p-3 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-[8px] font-black text-blue-400 uppercase tracking-tighter flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse" /> Live Telemetry
                    </span>
                    <Info className="w-3 h-3 text-slate-600" />
                </div>
                <div className="space-y-4">
                    {loads.filter(l => l.status === LOAD_STATUS.Active).slice(0, 3).map(l => (
                        <div key={l.id} className="p-3 bg-slate-900/50 rounded-xl border border-slate-800 flex items-center justify-between">
                            <div className="flex flex-col">
                                <span className="text-[8px] font-black text-slate-500 uppercase">Load #{l.loadNumber}</span>
                                <span className="text-xs font-bold text-white">{l.commodity || 'General Freight'}</span>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[8px] font-black text-blue-400 uppercase">v 62 MPH</span>
                                <span className="text-[8px] font-bold text-slate-400">ETA: 14:30</span>
                            </div>
                        </div>
                    ))}
                </div>
