...
import {
    Plus, Trash2, DollarSign, Briefcase,
    Truck, User, Package, Calculator, CheckCircle,
    X, Save, FilePlus, ChevronRight
} from 'lucide-react';
import {
    APBill, APBillLine, BillLineCategory,
    LineAllocationType, Vendor, LoadData, User as UserType
} from '../types';
...
export const AccountingBillForm: React.FC<Props> = ({ loads, onSave, onClose }) => {
    const [bill, setBill] = useState<Partial<APBill>>({
        ...
        status: 'Draft',
        lines: [
            { id: Math.random().toString(36).substr(2, 9), description: '', category: 'Labor', amount: 0, allocationType: 'Overhead', allocationId: 'SYSTEM', glAccountId: 'GL-6000', billId: 'pending' }
        ]
    });
    ...
    const getAllocationIcon = (type: LineAllocationType) => {
        switch (type) {
            case 'Load': return <Package className="w-3.5 h-3.5" />;
            case 'Truck':
            case 'Trailer': return <Truck className="w-3.5 h-3.5" />;
            case 'Driver': return <User className="w-3.5 h-3.5" />;
            case 'Overhead': return <Briefcase className="w-3.5 h-3.5" />;
        }
    };
                ...
                <div className="flex-1 overflow-auto p-10 space-y-10">
                    ...
                    <div className="grid grid-cols-4 gap-6">
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Bill Reference #</label>
                            <input
                                type="text"
                        ...
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Vendor Entity</label>
                            <select
                        ...
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Invoice Date</label>
                            <input
                                type="date"
                                className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase text-white outline-none focus:border-emerald-500/50"
                                value={bill.billDate}
                        ...
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Payment Due</label>
                            <input
                                type="date"
                                className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase text-white outline-none focus:border-emerald-500/50"
                                value={bill.dueDate}
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            <h3 className="text-xl font-black text-white uppercase tracking-tighter">Line Itemization</h3>
                            <button
                                onClick={addLine}
                        ...
                        <div className="bg-slate-950/50 border border-white/5 rounded-[2.5rem] overflow-hidden">
                            <table className="w-full text-left">
                                <thead className="bg-black/20 border-b border-white/5">
                                    <tr>
                                        <th className="px-6 py-4 text-[8px] font-black text-slate-600 uppercase">Description</th>
                                        <th className="px-6 py-4 text-[8px] font-black text-slate-600 uppercase">Category</th>
                                        <th className="px-6 py-4 text-[8px] font-black text-slate-600 uppercase">Allocation</th>
                                        <th className="px-6 py-4 text-[8px] font-black text-slate-600 uppercase">Alloc ID</th>
                                        <th className="px-6 py-4 text-[8px] font-black text-slate-600 uppercase text-right">Amount</th>
                                        ...
                                        <tr key={line.id} className="group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => updateLine(line.id, 'allocationType', e.target.value)}
                                                    >
                                            ...
                                            <td className="px-6 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) => updateLine(line.id, 'allocationId', e.target.value)}
                                                />
                                            </td>
                ...
                <div className="p-10 border-t border-white/5 bg-slate-900/20 shrink-0 flex justify-between items-center">
                    <div className="flex items-center gap-6">
                        <div className="px-8 py-4 bg-black/40 border border-white/5 rounded-2xl flex flex-col">
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">Total Bill Exposure</span>
                            <span className="text-3xl font-black text-white tracking-tighter">${bill.totalAmount?.toLocaleString()}</span>
                        </div>
