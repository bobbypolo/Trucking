...
import { History, Search, Filter, Shield, Activity, Truck, User as UserIcon, Calendar, ArrowRight, CheckCircle, AlertTriangle, Info } from 'lucide-react';
...
export const AuditLogs: React.FC<AuditLogsProps> = ({ loads, dispatchEvents, users }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [filterType, setFilterType] = useState<'all' | 'Status' | 'Alert' | 'System' | 'Ops'>('all');
    ...
    const allEvents = useMemo(() => {
        ...
        return [...loadLogs, ...dispatchLogs].sort((a, b) =>
    ...
    const filteredEvents = allEvents.filter(e => {
        ...
        const matchesFilter = filterType === 'all' || e.category === filterType || e.type === filterType;
        return matchesSearch && matchesFilter;
    });

    const getIcon = (category: string) => {
        switch (category) {
            case 'Status': return <CheckCircle className="w-4 h-4 text-emerald-500" />;
            case 'Alert': return <AlertTriangle className="w-4 h-4 text-amber-500" />;
            case 'StatusChange': return <Activity className="w-4 h-4 text-blue-500" />;
    ...
    return (
        <div className="h-full flex flex-col space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-white tracking-tighter uppercase mb-1">Audit Registry</h1>
                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">Immutable Transactional Governance Log</p>
                </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                {(['all', 'Status', 'Alert', 'System', 'Ops'] as const).map(f => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${filterType === f
                ...
                <div className="overflow-y-auto p-6 space-y-4 no-scrollbar">
                    {filteredEvents.length > 0 ? (
                        filteredEvents.map((event, idx) => (
                            <div key={event.id + idx} className="group relative pl-8 pb-4 border-l border-slate-800 last:pb-0">
                                ...
                                <div className="bg-slate-900 border border-slate-800/50 rounded-2xl p-4 hover:bg-slate-800/40 transition-all hover:border-slate-700/50">
                                    <div className="flex items-start justify-between gap-4 mb-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{event.category}</span>
                                            <span className="w-1 h-1 rounded-full bg-slate-700" />
                                            <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{event.entity}</span>
                                        </div>
                                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                                            {new Date(event.timestamp).toLocaleString()}
                                        </span>
                                    </div>
                                    <p className="text-xs font-bold text-slate-300 leading-relaxed mb-3">{event.message}</p>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">ACTOR: {event.user}</span>
                                    </div>
                                </div>
                            </div>
                        ))
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-12 text-center">
                            ...
                            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-1">No Activity Detected</h3>
                            <p className="text-xs text-slate-600 font-bold uppercase tracking-widest">Adjust filters or search criteria.</p>
                        </div>
                    )}
                </div>
            </div>
