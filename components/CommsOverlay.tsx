...
// Local extension of CallSession that carries a direct-dial phone number for
// the active call's primary contact. Kept local so this change does not touch
// shared types; CommsOverlay simply reads the optional `contactPhone` field
// when the caller provides one.
type CallSessionWithPhone = CallSession & { contactPhone?: string };
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${activeCallSession ? "bg-red-500/20 text-red-500" : "bg-blue-500/10 text-blue-500"}`}
            >
              <Phone
                className={`w-5 h-5 ${activeCallSession ? "animate-pulse" : ""}`}
              />
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${activeCallSession ? "bg-red-500" : "bg-slate-600"}`}
                />
                ...
                {activeCallSession &&
                  (activeCallSession as CallSessionWithPhone).contactPhone && (
                    <a
                      href={`tel:${(activeCallSession as CallSessionWithPhone).contactPhone}`}
                      className="text-[11px] font-bold text-blue-400 hover:text-blue-300 uppercase tracking-widest leading-none underline"
                      title={`Call ${(activeCallSession as CallSessionWithPhone).contactPhone}`}
                    >
                      {(activeCallSession as CallSessionWithPhone).contactPhone}
                    </a>
                  )}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                setOverlayState(
                  overlayState === "docked" ? "floating" : "docked",
                )
              }
              aria-label={
                overlayState === "docked" ? "Undock panel" : "Dock panel"
              }
                ...
                {filteredLoads.map((l) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="text-[10px] font-bold text-slate-500 group-hover:text-blue-100 uppercase">
                        {l.driverName} • {l.status}
                      </div>
                    </div>
                    {onLinkSessionToRecord && (
                      <button
              ...
              <div className="flex-1 space-y-4 mb-4">
                ...
                <div className="text-center py-4 border-b border-white/5 mb-6">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">
                    Session Started At{" "}
                    {new Date(activeCallSession.startTime).toLocaleTimeString()}
                  </span>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-slate-500"
                    >
          ...
          activeTab === "messages" && (
            <div className="h-full flex flex-col">
              <div className="flex-1 space-y-4 mb-4 overflow-y-auto no-scrollbar">
                {(
                  allLoads.find((l) => l.id === session.primaryContext?.id)
                    ?.messages || []
                ).length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center opacity-30">
                    <MessageSquare className="w-8 h-8 mb-2" />
                    <span className="text-[10px] font-black uppercase tracking-widest">
                      No Strategic Messages
                    </span>
                  </div>
                ) : (
                  (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold text-slate-600 uppercase mt-1 px-1">
                        {msg.senderName} •{" "}
            ...
            <div className="space-y-4">
              <div className="p-4 bg-blue-600/10 border border-blue-500/20 rounded-2xl mb-6">
                ...
                <p className="text-[11px] font-bold text-white uppercase">
                  {session.primaryContext?.label || "Direct Interaction"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    <div className="text-[11px] font-black text-white uppercase mb-1 group-hover:text-blue-400">
                      {req.label}
                    </div>
