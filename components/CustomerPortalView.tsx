...
import {
    Package, FileText, LifeBuoy, Search, Filter,
    ArrowRight, MapPin, Clock, Download, Plus,
    Truck, CheckCircle2, Navigation, AlertCircle, Globe, MessageSquare
} from 'lucide-react';
...
export const CustomerPortalView: React.FC<Props> = ({ user, loads, onOpenHub }) => {

    const [activeView, setActiveView] = useState('shipments');
    const [selectedLoadId, setSelectedLoadId] = useState<string | null>(null);
    const [searchTerm, setSearchTerm] = useState('');

    const customerLoads = loads.filter(l => l.customerUserId === user.id || l.brokerId === user.id);
    const filteredLoads = customerLoads.filter(l =>
            ...
            <div className="flex h-full overflow-hidden">
                ...
                <aside className="w-64 bg-[#0a0f1e] border-r border-white/5 flex flex-col p-6 space-y-8 hidden md:flex shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-black tracking-tighter uppercase">Client Portal</span>
                    </div>
                    ...
                    <nav className="space-y-1">
                        {[
                            ...
                            <button
                                key={item.id}
                                onClick={() => { setActiveView(item.id); setSelectedLoadId(null); }}
                    ...
                    <div className="mt-auto p-4 bg-slate-950/50 rounded-2xl border border-white/5">
                        <div className="text-[8px] font-black text-slate-600 uppercase mb-2">Logged in as</div>
                        <div className="text-[10px] font-black text-white truncate uppercase">{user.name}</div>
                        <div className="text-[8px] font-bold text-slate-500 uppercase mt-0.5">{user.company}</div>
                    </div>
                </aside>
                ...
                <main className="flex-1 flex flex-col min-w-0 bg-[#020617]">
                    <header className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-[#0a0f1e]/50 shrink-0">
                        <div className="flex items-center gap-4">
                            {selectedLoadId && (
                                <button onClick={() => setSelectedLoadId(null)} className="text-slate-500 hover:text-white mr-2">
                                    <ArrowRight className="w-5 h-5 rotate-180" />
                                </button>
                            )}
                            <h2 className="text-xs font-black text-white uppercase tracking-widest">
                                {activeView === 'shipments' && (selectedLoadId ? `Tracking Load #${selectedLoad?.loadNumber}` : 'Shipment Visibility')}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                <div className="space-y-1">
                                                    <div className="text-[8px] font-black text-blue-500 uppercase tracking-widest">Load #{load.loadNumber}</div>
                                                    <div className="text-sm font-black text-white uppercase tracking-tight line-clamp-1">{load.pickup.city} → {load.dropoff.city}</div>
                                                </div>
                                                <div className="bg-slate-950 px-2 py-1 rounded-lg border border-white/5">
                                                    <span className="text-[8px] font-black text-blue-400 uppercase">{load.status}</span>
                                                </div>
                                            </div>
                                    ...
                                    {filteredLoads.length === 0 && (
                                        <div className="col-span-full py-20 text-center bg-[#0a0f1e]/40 rounded-[2rem] border border-dashed border-white/5">
                                            ...
                                            <p className="text-xs font-black text-slate-600 uppercase tracking-[0.2em]">No shipments matching your search</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                                    ...
                                    <div className="relative z-10 space-y-12">
                                        <div className="flex justify-between items-start">
                                            <div className="space-y-3">
                                                ...
                                                <h1 className="text-4xl font-black text-white uppercase tracking-tighter">
                                                    {selectedLoad.pickup.city} <ArrowRight className="inline-block w-8 h-8 text-blue-500 mx-2" /> {selectedLoad.dropoff.city}
                                                </h1>
                                            </div>
                                            <div className="text-right space-y-1">
                                                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Current Status</div>
                                                <div className="text-2xl font-black text-blue-400 uppercase tracking-tight">{selectedLoad.status}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                            <div className="bg-slate-950/50 p-6 rounded-3xl border border-white/5 space-y-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-blue-600/10 rounded-xl"><MapPin className="w-4 h-4 text-blue-500" /></div>
                                                    <span className="text-[10px] font-black text-slate-500 uppercase">Location</span>
                                                </div>
                                                <div className="text-sm font-black text-white uppercase">Syracuse, NY</div>
                                            </div>
                                            <div className="bg-slate-950/50 p-6 rounded-3xl border border-white/5 space-y-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-indigo-600/10 rounded-xl"><Clock className="w-4 h-4 text-indigo-500" /></div>
                                                    <span className="text-[10px] font-black text-slate-500 uppercase">Distance left</span>
                                                </div>
                                                <div className="text-sm font-black text-white uppercase">142 Miles</div>
                                            </div>
                                            <div className="bg-slate-950/50 p-6 rounded-3xl border border-white/5 space-y-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-green-600/10 rounded-xl"><CheckCircle2 className="w-4 h-4 text-green-500" /></div>
                                                    <span className="text-[10px] font-black text-slate-500 uppercase">Condition</span>
                                                </div>
                                                <div className="text-sm font-black text-white uppercase">In Optimal SLA</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="h-3 w-full bg-slate-950 rounded-full border border-white/5 overflow-hidden">
                                                <div className="h-full bg-blue-600 rounded-full shadow-[0_0_20px_#2563eb]" style={{ width: '78%' }} />
                                            </div>
                                        </div>
                        ...
                        {activeView === 'invoices' && (
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-2xl font-black text-white uppercase">Invoice History</h3>
                                    ...
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="border-b border-white/5 bg-slate-950/50">
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Invoice #</th>
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Load #</th>
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Date</th>
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Amount</th>
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Status</th>
                                                <th className="px-8 py-5 text-[9px] font-black text-slate-500 uppercase tracking-widest text-right">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-white/5">
                                            {[1, 2, 3, 4, 5].map(i => (
                                                <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                                                    <td className="px-8 py-5 text-[10px] font-black text-white">INV-8409{i}</td>
                                                    <td className="px-8 py-5 text-[10px] font-bold text-slate-500">TRK-491{i}</td>
                                                    <td className="px-8 py-5 text-[10px] font-bold text-slate-500">2023-12-1{i}</td>
                                                    <td className="px-8 py-5 text-[10px] font-black text-white">$4,250.00</td>
                                                    <td className="px-8 py-5">
                                                        <span className="px-2 py-0.5 bg-green-500/20 text-green-500 rounded-lg text-[8px] font-black uppercase border border-green-500/20">Paid</span>
                                                    </td>
                        ...
                        {activeView === 'quotes' && (
                            <div className="max-w-2xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4">
                                <div className="text-center space-y-4">
                                    ...
                                    <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Freight Quote Request</h3>
                                    <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Priority lane pricing in seconds</p>
                                </div>

                                <form className="bg-[#0a0f1e] border border-white/10 rounded-[2.5rem] p-10 space-y-8 shadow-2xl">
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Origin City</label>
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Destination City</label>
                                            <input className="w-full bg-slate-950 border border-white/5 rounded-2xl px-6 py-4 text-xs font-bold text-white focus:border-blue-500 outline-none transition-all" />
                                        </div>
                                    </div>
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Equipment Type</label>
                                    ...
                                    <button type="button" className="w-full py-5 bg-blue-600 text-white rounded-3xl text-sm font-black uppercase tracking-[0.2em] shadow-2xl shadow-blue-900/40 hover:bg-blue-500 transition-all active:scale-95">
                        ...
                        {activeView === 'support' && (
                            <div className="h-full flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-4">
                                <div className="flex items-center justify-between">
                                    <div className="space-y-1">
                                        <h3 className="text-2xl font-black text-white uppercase tracking-tight">Support Desk</h3>
                                        <p className="text-sm text-slate-500 font-bold uppercase tracking-widest">Direct channel to operations</p>
                                    </div>
                                ...
                                <div className="flex-1 bg-slate-950/30 rounded-[2.5rem] border border-white/5 overflow-hidden">
                                    <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-6 p-12 text-center">
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-xs font-black uppercase tracking-widest text-white">Direct Operational Stream</p>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">Messaging has transitioned to Central Command for firm-wide coordination.</p>
                                        </div>
                                        <button
