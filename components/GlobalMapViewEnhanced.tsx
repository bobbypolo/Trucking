...
import {
    Truck, MapPin, Navigation, Map as MapIcon,
    Layers, Maximize2, X, Phone, AlertCircle,
    Wifi, WifiOff, Cloud, CloudRain, CloudSnow,
    Sun, Wind, PlayCircle, Package, Search,
...
const WEATHER_API_KEY = (import.meta as any).env.VITE_WEATHER_API_KEY || 'YOUR_WEATHER_API_KEY';
...
export const GlobalMapViewEnhanced: React.FC<Props> = ({
    loads, users, incidents = [], onViewLoad, onSelectIncident,
    isHighObstruction = false, obstructionLevel = 'NOMINAL',
    showSideOverlays = true
}) => {
    const [libraries] = useState<("geometry" | "drawing" | "places")[]>(['geometry', 'drawing', 'places']);
    ...
    const [localOverlaysVisible, setLocalOverlaysVisible] = useState(showSideOverlays);

    useEffect(() => {
        setLocalOverlaysVisible(showSideOverlays);
    }, [showSideOverlays]);

    // Mock vehicle data generation based on users
    const activeVehicles: VehicleMarker[] = useMemo(() => {
        return users.filter(u => u.role === 'driver').map(driver => {
            // Find if driver has an active load
            const activeLoad = loads.find(l => l.driverId === driver.id && (l.status === 'In Transit' || l.status === 'Active'));
            const hasIncident = incidents.some(inc => inc.driverId === driver.id && inc.status !== 'Closed');

            // Deterministic random position for demo
            const seed = driver.id.split('-').reduce((acc, char) => acc + char.charCodeAt(0), 0);
            const lat = 35 + (seed % 10);
            const lng = -100 + (seed % 20);
    ...
    const filteredLoads = useMemo(() => {
        ...
        return loads.filter(l => {
            if (loadFilter === 'In Transit') return l.status === 'In Transit' || l.status === 'Active';
    ...
    const filteredVehicles = useMemo(() => {
        return activeVehicles.filter(v => {
            const matchesSearch = v.driver.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const getWeatherIcon = () => {
        ...
        if (condition.includes('snow')) return <CloudSnow className="w-4 h-4 text-blue-200" />;
        if (condition.includes('clear') || condition.includes('sunny')) return <Sun className="w-4 h-4 text-yellow-400" />;
        return <Cloud className="w-4 h-4 text-slate-400" />;
    };

    const getLoadStatusIcon = (status: LoadStatus) => {
        switch (status) {
            case LOAD_STATUS.In_Transit: return <Navigation className="w-3 h-3 text-blue-400" />;
            case LOAD_STATUS.Booked: return <Package className="w-3 h-3 text-yellow-400" />;
            case LOAD_STATUS.Unassigned: return <AlertCircle className="w-3 h-3 text-slate-400" />;
            case LOAD_STATUS.Delivered: return <Activity className="w-3 h-3 text-emerald-400" />;
    ...
    useEffect(() => {
        const fetchRoutes = async () => {
            for (const load of loads) {
                if ((load.status === LOAD_STATUS.Active || load.status === LOAD_STATUS.In_Transit) && !routePaths[load.id]) {
                    try {
                        ...
                        if (typeof google !== 'undefined' && google.maps.geometry) {
                            const decodedPath = google.maps.geometry.encoding.decodePath(directions.points);
                            const points = decodedPath.map(p => ({ lat: p.lat(), lng: p.lng() }));
    ...
    return (
        <div className="flex-1 relative overflow-hidden group/map w-full h-full">
                        ...
                        {filteredVehicles.map((vehicle) => (
                            <Marker
                                ...
                                icon={typeof google !== 'undefined' ? {
                                    ...
                                    fillColor: vehicle.hasIncident ? '#ef4444' : vehicle.isOnline ? '#3b82f6' : '#64748b',
                                    fillOpacity: 1,
                                    strokeColor: '#ffffff',
                        ...
                        {selectedVehicle && (
                            ...
                            >
                                <div className="p-2 min-w-[200px]">
                                    <div className="text-[10px] font-black text-blue-600 uppercase mb-1">Driver Profile</div>
                                    <div className="text-sm font-black text-slate-900 mb-2">{selectedVehicle.driver.name}</div>
                                    <div className="flex items-center gap-2 mb-3">
                                        <div className={`w-2 h-2 rounded-full ${selectedVehicle.isOnline ? 'bg-green-500' : 'bg-red-500'}`} />
                                        <span className="text-[10px] font-bold text-slate-500 uppercase">{selectedVehicle.isOnline ? 'Connected' : 'Offline'}</span>
                                    </div>
                                    {selectedVehicle.activeLoad && (
                                        <div className="bg-slate-100 rounded-lg p-2 mb-3 border border-slate-200">
                                            <div className="text-[8px] font-black text-slate-500 uppercase mb-1">Active Load</div>
                                            <div className="text-[10px] font-bold text-blue-600">#{selectedVehicle.activeLoad.loadNumber}</div>
                                            <div className="text-[9px] text-slate-600 italic">
                                                {selectedVehicle.activeLoad.pickup.city} → {selectedVehicle.activeLoad.dropoff.city}
                                            </div>
                                        </div>
                                    )}
                                    <button
            ...
            {weather && !isHighObstruction && (
                <div className="absolute top-8 right-8 z-20 bg-[#0a0f1e]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl animate-in slide-in-from-right duration-500">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">Operational Weather</span>
                        {getWeatherIcon()}
                    </div>
                    <div className="flex items-center gap-3">
                        <div className={`font-black text-white leading-none ${isHighObstruction ? 'text-4xl' : 'text-3xl'}`}>{weather.temp}°</div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-slate-300">{weather.condition}</div>
                            <div className="flex items-center gap-2 mt-1">
                                <Wind className="w-3 h-3 text-slate-500" />
                                <span className="text-[9px] text-slate-500">{weather.windSpeed} mph</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {localOverlaysVisible && (
                <div className={`absolute top-8 left-8 z-20 space-y-4 transition-all duration-500 transform ${isHighObstruction ? '-translate-x-[110%] opacity-0' : 'translate-x-0 opacity-100'} w-80`}>
                    <div className="bg-[#0a0f1e]/90 backdrop-blur-xl border border-red-500/20 rounded-2xl p-5 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black text-white uppercase tracking-tighter">Active Incidents</span>
                            </div>
                            <span className="bg-red-500/10 text-red-500 px-2 py-0.5 rounded-lg text-[8px] font-black">{incidents.filter(i => i.status !== 'Closed').length}</span>
                        </div>
                        <div className="space-y-3 max-h-48 overflow-y-auto no-scrollbar">
                            {incidents.filter(i => i.status !== 'Closed').map(inc => (
                                ...
                                >
                                    <div className="text-[9px] font-black text-white uppercase mb-1">{inc.type}</div>
                                    <div className="text-[8px] text-slate-500 truncate">{inc.description}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0a0f1e]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">Fleet Filters</span>
            ...
            {localOverlaysVisible && (
                <div className={`absolute top-8 right-8 z-20 space-y-4 transition-all duration-500 transform ${rightPanelCollapsed || (isHighObstruction && !searchTerm) ? 'translate-x-[110%] opacity-0' : 'translate-x-0 opacity-100'} ${isHighObstruction ? 'w-64' : 'w-80'} ${weather ? 'mt-32' : ''}`}>
                    <div className="bg-[#0a0f1e]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl">
                        <div className="flex justify-between items-center mb-4">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">Driver Fleet</span>
                                ...
                                >
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-white truncate max-w-[140px] uppercase tracking-tight italic">{vehicle.driver.name}</span>
                                        </div>
                                        {vehicle.hasIncident && <AlertCircle className="w-3 h-3 text-red-500 animate-pulse" />}
                                    </div>
                                    <div className="flex items-center justify-between text-[8px] text-slate-500 font-bold uppercase tracking-widest">
                                        <span>{vehicle.activeLoad ? `Load #${vehicle.activeLoad.loadNumber}` : 'Available'}</span>
                ...
                <div className="absolute inset-y-8 right-8 w-96 z-30 bg-[#050810]/98 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.8)] p-8 animate-in slide-in-from-right duration-500 overflow-hidden flex flex-col">
                    ...
                    <div className="flex items-center gap-6 mb-10">
                        <div className="w-20 h-20 bg-slate-800 rounded-3xl flex items-center justify-center border border-white/10 overflow-hidden relative">
                            {selectedDriverOverlay.driver.avatar ? (
                                <img src={selectedDriverOverlay.driver.avatar} alt="" className="w-full h-full object-cover" />
                            ) : (
                                <Truck className="w-8 h-8 text-slate-600" />
                            )}
                            <div className={`absolute bottom-2 right-2 w-3.5 h-3.5 rounded-full border-2 border-slate-900 ${selectedDriverOverlay.isOnline ? 'bg-green-500' : 'bg-red-500'}`} />
                        </div>
                        <div>
                            <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">{selectedDriverOverlay.driver.name}</h3>
                            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Ref ID: {selectedDriverOverlay.driver.id.slice(0, 8)}</div>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 mb-8">
                        <div className="bg-slate-950/60 p-4 rounded-2xl border border-white/5">
                            <div className="text-[8px] font-black text-slate-600 uppercase mb-1">Safety Score</div>
                            <div className="text-[11px] font-black text-emerald-500 uppercase">98%</div>
                        </div>
                        <div className="bg-slate-950/60 p-4 rounded-2xl border border-white/5">
                            <div className="text-[8px] font-black text-slate-600 uppercase mb-1">Last Ping</div>
                            <div className="text-[11px] font-black text-white uppercase">
                                {new Date(selectedDriverOverlay.lastPing).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </div>
                        </div>
                    </div>
                    ...
                    {selectedDriverOverlay.activeLoad && (
                        <div className="bg-blue-600/10 border border-blue-500/20 rounded-2xl p-4 mb-6">
                            <div className="text-[10px] font-black text-blue-400 uppercase mb-2">Current Load</div>
                            <div className="text-[12px] font-bold text-white mb-1">#{selectedDriverOverlay.activeLoad.loadNumber}</div>
                            <div className="text-[10px] text-slate-400">
                                {selectedDriverOverlay.activeLoad.pickup.city} → {selectedDriverOverlay.activeLoad.dropoff.city}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => window.location.href = `tel:${selectedDriverOverlay.driver.phone || ''}`}
