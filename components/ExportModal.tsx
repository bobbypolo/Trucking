...
export const ExportModal: React.FC<Props> = ({
  ...
  const handleExport = (type: "pdf" | "csv") => {
    const config = {
      ...
      includeTotals:
        selectedColumns.includes("rate") || selectedColumns.includes("miles"),
    };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div
        ref={panelRef}
        className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        <div className="p-5 border-b border-slate-700 flex justify-between items-center bg-slate-900/50 rounded-t-xl">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Generate custom reports for accounting or analysis.
            </p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-900 p-3 rounded border border-slate-700">
                <label className="text-xs text-slate-400 mb-2 block font-semibold">
                  Date Range
                </label>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => setMonth(0)}
                    className="text-[10px] bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-blue-300 border border-slate-600"
                  >
                    This Month
                  </button>
                  <button
                    onClick={() => setMonth(-1)}
                    className="text-[10px] bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-300 border border-slate-600"
                  >
                    Last Month
                  </button>
                  <button
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-bold text-white flex items-center gap-2 uppercase tracking-wide">
              <CheckSquare className="w-4 h-4 text-green-400" /> 2. Select
              Columns
            </h3>
