...
import {
    MessageSquare, Send, Truck, User, Clock,
    ArrowLeft, MoreVertical, Paperclip, Smile, Search as SearchIcon,
    MapPin, Calendar, Phone, Info, CheckSquare, PlusSquare, Users as UsersIcon, Link, CreditCard, AlertTriangle
} from 'lucide-react';
import { User as UserType, LoadData, Message, GlobalSearchResult, CallSession, OperationalEvent, WorkspaceSession } from '../types';
import { getMessages, saveMessage, globalSearch } from '../services/storageService';
...
interface Props {
    ...
    onLinkSession?: (sessionId: string, recordId: string, recordType: any) => Promise<void>;
...
export const OperationalMessaging: React.FC<Props> = ({
    user, loads, initialLoadId, onClose, callSession, interactionState = 'IDLE', onNoteCreated, onRecordAction, onLinkSession, session, unifiedEvents = [], threads = []
}) => {
    const activeContext = session.primaryContext;
    const [selectedThreadId, setSelectedThreadId] = useState<string | null>(initialLoadId || (activeContext?.type === 'LOAD' ? activeContext.id : activeContext?.type === 'INCIDENT' ? `inc-${activeContext.id}` : null) || threads[0]?.id || loads[0]?.id || null);
    const [searchQuery, setSearchQuery] = useState('');
    const [messageText, setMessageText] = useState('');
    const [noteText, setNoteText] = useState('');
    const [activeViewTab, setActiveViewTab] = useState<'MESSAGES' | 'NOTES'>('MESSAGES');
    const [showContext, setShowContext] = useState(true);
    const [participantSearch, setParticipantSearch] = useState('');
    const [participantResults, setParticipantResults] = useState<GlobalSearchResult[]>([]);
    ...
    const selectedLoad = useMemo(() =>
        loads.find(l => l.id === selectedThreadId || `inc-${l.id}` === selectedThreadId),
    ...
    const filteredThreads = (threads.length > 0 ? threads : []).filter(t =>
        (t.primaryContext?.label || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.summary || '').toLowerCase().includes(searchQuery.toLowerCase())
    );

    const activeMessages = messages.filter(m => m.loadId === selectedThreadId || `inc-${m.loadId}` === selectedThreadId);
    ...
    const handleAddParticipant = (res: GlobalSearchResult) => {
        ...
        setParticipantResults([]);
        if (onNoteCreated) onNoteCreated(`PARTICIPANT: Linked ${res.label} (${res.type}) to Liaison Stream.`);
    };
    ...
    const handleCreateTask = async (text: string) => {
        ...
        if (onNoteCreated) onNoteCreated(`ACTION: Created Strategic Task - ${text}`);
    ...
    const handleQuickRequest = async (type: string) => {
        ...
        if (onNoteCreated) onNoteCreated(`ACTION: Committing ${type} request to Ledger`);
    };
            ...
            <aside className="w-80 border-r border-white/5 flex flex-col shrink-0 bg-[#0a0f1e]/50">
                <div className="p-6 border-b border-white/5 space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Operational Streams</h2>
                        <button className="p-2 hover:bg-white/5 rounded-lg text-slate-500" title="Manage Threads"><MoreVertical className="w-4 h-4" /></button>
                    </div>
                    <div className="bg-blue-500/5 border border-blue-500/10 rounded-xl p-3 mb-2">
                        <p className="text-[8px] font-bold text-blue-400 uppercase leading-relaxed">
                            These are record-linked liaison streams. For live inbound calls, use the <span className="underline">Live Comm Queue</span>.
                        </p>
                    </div>
                        ...
                        >
                            <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center border border-white/5 shrink-0">
                                {thread.id.startsWith('inc-') ? (
                                    <AlertTriangle className={`w-5 h-5 ${selectedThreadId === thread.id ? 'text-red-500' : 'text-slate-600'}`} />
                                ) : (
                                    <Truck className={`w-5 h-5 ${selectedThreadId === thread.id ? 'text-blue-500' : 'text-slate-600'}`} />
                                )}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="text-[10px] font-black text-white uppercase tracking-tighter">{thread.primaryContext?.label}</span>
                                    {selectedThreadId === thread.id && <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />}
                                </div>
                                ...
                                <div className="mt-2 text-[9px] text-slate-400 truncate opacity-60">
                                    {messages.findLast(m => m.loadId === thread.id || `inc-${m.loadId}` === thread.id)?.text || 'No updates recently'}
                                </div>
                            </div>
                        </button>
                    ))}
                    ...
                    <>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-xs font-black uppercase text-white tracking-tight">
                                            {selectedLoad ? `Load #${selectedLoad.loadNumber} Liaison` : 'Operational Messaging'}
                                        </h3>
                                        <div className="flex items-center gap-3 mt-1">
                                            <div className={`w-1.5 h-1.5 rounded-full ${interactionState === 'ACTIVE' ? 'bg-green-500 animate-pulse' : 'bg-slate-500'}`} />
                                            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest leading-none">
                                                {interactionState} {callSession?.id ? `(${callSession.id})` : ''}
                                            </span>
                                            {interactionState === 'ACTIVE' && (
                                                <button
                                                    onClick={() => onNoteCreated?.("Interaction Ended via Liaison")}
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                                ...
                                <div className="bg-blue-600/10 border border-blue-500/30 rounded-3xl p-6 mb-8 animate-in slide-in-from-top duration-500">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Live Voice Interaction</h4>
                                                <p className="text-[8px] font-bold text-blue-500 uppercase tracking-tighter mt-0.5">Session ID: {callSession?.id || 'PENDING'}</p>
                                            </div>
                                        </div>
                                        <div className="bg-slate-900/50 px-3 py-1 rounded-lg border border-white/10">
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Active Partner: {callSession?.participants?.[0]?.name || 'Unknown'}</span>
                                        </div>
                                    </div>
                                    <div className="relative">
                                        <textarea
                            ...
                            {[
                                ...
                                ...(unifiedEvents || []).filter(e => (e.loadId === selectedThreadId || (e.loadId && `inc-${e.loadId}` === selectedThreadId))).map(e => ({ ...e, streamType: 'EVENT' }))
                            ].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((item: any) => (
                                <div key={item.id} className={`flex flex-col ${item.streamType === 'MESSAGE' && item.senderId === user.id ? 'items-end' : 'items-start'}`}>
                                    {item.streamType === 'MESSAGE' ? (
                                        <>
                                            ...
                                            <div className={`max-w-[70%] p-4 rounded-2xl text-[11px] font-medium leading-relaxed ${item.senderId === user.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'bg-slate-900/80 text-slate-300 border border-white/5'}`}>
                                                {item.text}
                                            </div>
                                        </>
                                    ) : (
                                        <div className="w-full flex items-center gap-4 py-2 opacity-80 group">
                                            ...
                                            <div className="flex items-center gap-3 px-4 py-2 bg-slate-900/40 border border-white/5 rounded-full">
                                                <div className={`p-1.5 rounded-md ${item.type === 'CALL_LOG' ? 'bg-blue-500/10 text-blue-500' : 'bg-slate-500/10 text-slate-500'}`}>
                                                    {item.type === 'CALL_LOG' ? <Phone className="w-3 h-3" /> : <Info className="w-3 h-3" />}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-white uppercase tracking-tight">{item.message}</span>
                                                    <span className="text-[7px] font-bold text-slate-600 uppercase tracking-widest">
                                                        {item.actorName} • {new Date(item.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="h-px flex-1 bg-white/5" />
                        ...
                        {activeViewTab === 'MESSAGES' && (
                            <div className="p-6 border-t border-white/5">
                                <div className="bg-slate-950 border border-white/5 rounded-2xl p-2 flex items-end gap-2 shadow-inner">
                                    <button className="p-3 text-slate-600 hover:text-slate-400 transition-colors"><Smile className="w-5 h-5" /></button>
                                    <button className="p-3 text-slate-600 hover:text-slate-400 transition-colors"><Paperclip className="w-5 h-5" /></button>
                                    <textarea
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-20 text-center bg-slate-950/20 backdrop-blur-sm">
                        {interactionState === 'ACTIVE' ? (
                            <div className="bg-slate-900 border border-white/5 rounded-[3rem] p-12 max-w-md animate-in fade-in zoom-in duration-500 shadow-2xl">
                                ...
                                <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-4">Unlinked Active Session</h3>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-8 leading-relaxed">
                                    You have an active interaction ({callSession?.id}) without a linked operational record. Search for a load or driver to associate this session.
                                </p>
                                    ...
                                    {participantResults.length > 0 && (
                                        <div className="bg-slate-950/80 border border-white/5 rounded-2xl overflow-hidden max-h-60 overflow-y-auto no-scrollbar shadow-2xl">
                                            {participantResults.map(res => (
                                                <button
                                                    key={res.id}
                                                    onClick={() => onLinkSession?.(callSession!.id, res.id, res.type as any)}
                                                ...
                                                >
                                                    <div className="flex flex-col text-left">
                                                        <span className="text-[11px] font-black text-white uppercase group-hover:text-blue-400">{res.label}</span>
                                                        <span className="text-[8px] font-bold text-slate-500 uppercase">{res.type} • {res.subLabel}</span>
                                                    </div>
                                                    <div className="px-3 py-1 bg-blue-600/10 text-blue-500 text-[8px] font-black uppercase rounded opacity-0 group-hover:opacity-100">Link Session</div>
                                                </button>
                                            ))}
                                        </div>
                                    )}
                            ...
                            <div className="opacity-40 flex flex-col items-center">
                                ...
                                <h3 className="text-xl font-black text-white uppercase tracking-tighter mb-2">Omni-Channel Liaison</h3>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest max-w-xs">Select a communication thread from the left rail to initiate tactical messaging.</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            {showContext && selectedLoad && (
                <aside className="w-72 border-l border-white/5 flex flex-col shrink-0 bg-[#0a0f1e]/80 animate-in slide-in-from-right duration-200">
                    <div className="p-6 border-b border-white/5">
                        <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Resource Context</h2>
                    </div>
                    <div className="p-6 space-y-8 overflow-y-auto no-scrollbar">
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-white">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-tighter">Routing Geometry</div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-slate-950 rounded-2xl border border-white/5">
                                    <div className="text-[8px] font-black text-slate-500 uppercase mb-1">Pickup</div>
                                    <div className="text-[10px] font-bold text-white truncate">{selectedLoad.pickup.city}</div>
                                </div>
                                <div className="p-4 bg-slate-950 rounded-2xl border border-white/5">
                                    <div className="text-[8px] font-black text-slate-500 uppercase mb-1">Dropoff</div>
                                    <div className="text-[10px] font-bold text-white truncate">{selectedLoad.dropoff.city}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-white">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-tighter">Temporal Data</div>
                            </div>
                            <div className="space-y-3">
                                <div className="flex justify-between items-center text-[10px]">
                                    <span className="text-slate-500 font-bold uppercase tracking-widest">Pickup Date</span>
                                    <span className="text-white font-black">{selectedLoad.pickupDate}</span>
                                </div>
                                <div className="flex justify-between items-center text-[10px]">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-white">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-tighter">Operational Tasks</div>
                            </div>
                            <div className="space-y-2">
                                {tasks.map(task => (
                                    <div key={task.id} className="flex items-center gap-3 p-3 bg-slate-950 rounded-xl border border-white/5 active:scale-95 transition-all cursor-pointer" onClick={() => handleToggleTask(task.id)}>
                                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${task.completed ? 'bg-green-500 border-green-500' : 'border-white/20'}`}>
                                            {task.completed && <CheckSquare className="w-3 h-3 text-white" />}
                                        </div>
                                        <span className={`text-[10px] uppercase font-bold tracking-tight ${task.completed ? 'text-slate-600 line-through' : 'text-slate-300'}`}>{task.text}</span>
                                    </div>
                                ))}
                                {isAddingTask ? (
                                    <div className="space-y-2 p-3 bg-slate-900 rounded-xl border border-blue-500/30 animate-in fade-in slide-in-from-top-2 duration-200">
                                        ...
                                        <div className="flex justify-between items-center">
                                            <span className="text-[7px] font-black text-slate-600 uppercase">Press Enter to Commit</span>
                                            <button onClick={() => { setIsAddingTask(false); setNewTaskText(''); }} className="text-[7px] font-black text-red-500 uppercase">Cancel</button>
                                        </div>
                                    </div>
                                ) : (
                                    <button
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-white">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-tighter">Quick Requests</div>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-white">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-tighter">Participants</div>
                            </div>
                            <div className="space-y-3">
                                <div className="relative">
                                    <SearchIcon className={`absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 ${isSearchingParticipants ? 'text-blue-500 animate-pulse' : 'text-slate-600'}`} />
                                    <input
                                        type="text"
                                    ...
                                    {participantResults.length > 0 && (
                                        <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden max-h-48 overflow-y-auto">
                                            {participantResults.map(res => (
                                                ...
                                                >
                                                    <div>
                                                        <div className="text-[9px] font-black text-white uppercase">{res.label}</div>
                                                        <div className="text-[7px] font-bold text-slate-500 uppercase">
                                                            {res.type} {res.subLabel ? `• ${res.subLabel}` : ''}
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        {res.type === 'DRIVER' && <Phone className="w-3 h-3 text-emerald-500" />}
                                ...
                                <div className="flex -space-x-2">
                                    <div className="w-6 h-6 rounded-full bg-blue-600 border-2 border-[#10141d] flex items-center justify-center text-[8px] font-black text-white" title={user.name}>{user.name.charAt(0)}</div>
                                    {[1, 2].map(i => (
                                        <div key={i} className="w-6 h-6 rounded-full bg-slate-800 border-2 border-[#10141d] flex items-center justify-center text-[8px] font-black text-slate-500">?</div>
                                    ))}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 space-y-3">
                            <button className="w-full flex items-center justify-between p-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl transition-all shadow-lg shadow-blue-900/20 group">
                                <span className="text-[10px] font-black uppercase tracking-widest">Contact Dispatch</span>
