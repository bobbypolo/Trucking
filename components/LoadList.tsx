...
export const LoadList: React.FC<Props> = ({
  loads,
  onView,
  onEdit,
  onDelete,
  ...
  const processedLoads = useMemo(() => {
    let result = [...loads];
    if (isDispatcher && viewScope === "my")
      result = result.filter((l) => l.dispatcherId === currentUser?.id);
    if (filter) {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    const baseStyle =
      "text-[10px] px-2 py-0.5 rounded uppercase tracking-[0.2em] font-black border";
    ...
    return (
      <span
        className={`${baseStyle} ${colors[status] || "bg-slate-900 text-slate-400 border-slate-800"}`}
      >
        {status}
      </span>
    );
      ...
      <div className="p-4 bg-slate-900 border-b border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 shadow-lg relative z-10">
        ...
        <div className="flex gap-3">
          {isDispatcher && (
            <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800 shadow-inner">
              <button
                ...
                className={`px-5 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${viewScope === "my" ? "bg-blue-600 text-white shadow-xl" : "text-slate-600 hover:text-slate-300"}`}
              >
                Assigned Deck
              </button>
              <button
                onClick={() => setViewScope("all")}
                className={`px-5 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${viewScope === "all" ? "bg-blue-600 text-white shadow-xl" : "text-slate-600 hover:text-slate-300"}`}
              >
                Fleet Network
              </button>
            </div>
          ...
          <button
            ...
            className="bg-slate-900 border border-slate-800 p-3 rounded-xl hover:bg-slate-800 transition-all text-slate-400"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar pb-24">
        ...
        {!isLoading &&
          ...
          processedLoads.map((load) => (
            <div
              key={load.id}
              className={`bg-slate-900 p-4 rounded-2xl border ${
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-xl bg-slate-950 flex items-center justify-center font-black ${load.isActionRequired ? "text-yellow-500" : "text-blue-500"} border border-slate-800 shadow-2xl group-hover:bg-blue-900/10 transition-all`}
                  >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {load.isActionRequired && (
                        <span className="bg-yellow-500/10 text-yellow-500 text-[10px] font-black px-1.5 py-0.5 rounded border border-yellow-500/30 uppercase tracking-[0.2em] flex items-center gap-1">
                          <AlertTriangle className="w-2.5 h-2.5" /> Action
                          Required
                        </span>
                      )}
                    </div>
                    <div className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
                      ...
                      <span className="flex items-center gap-1">
                        <UserIcon className="w-2.5 h-2.5 text-slate-800" />{" "}
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-950/50 p-4 rounded-xl border border-slate-800/50 group-hover:border-slate-700/50 transition-all relative overflow-hidden">
                <div className="space-y-1.5 border-r border-slate-800 pr-4 relative z-10">
                  <div className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] flex items-center gap-1.5">
                    Origin
                  </div>
                  <div className="text-[11px] font-black text-slate-200 uppercase truncate tracking-wider">
                    {load.pickup?.city ?? ""}, {load.pickup?.state ?? ""}
                  </div>
                  <div className="text-[10px] font-bold text-slate-600 truncate uppercase mt-0.5">
              ...
              <div className="mt-4 flex justify-between items-center px-1">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-800 hover:bg-blue-600 text-slate-500 hover:text-white rounded-lg text-[11px] font-black uppercase tracking-widest transition-all"
                  >
                    <Edit2 className="w-3 h-3 inline mr-1.5" /> Modify
                  </button>
                </div>
              </div>
            </div>
