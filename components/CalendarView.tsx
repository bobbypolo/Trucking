...
const DayCell: React.FC<DayCellProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`flex justify-between items-center mb-1 ${isToday ? "text-blue-400" : "text-slate-400"}`}
      >
        <span
          className={`text-sm font-bold ${isToday ? "bg-blue-900/50 px-2 py-0.5 rounded-full border border-blue-500/30" : ""}`}
        >
          {date.getDate()}
        </span>
...
const MonthSection: React.FC<MonthSectionProps> = ({
  ...
  return (
    <div
      ref={(el) => onRegisterRef(`${year}-${month}`, el)}
    ...
    >
      ...
      <div className="grid grid-cols-7 border-l border-slate-700">
        ...
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div
            key={day}
            className="text-center text-[10px] uppercase font-bold text-slate-600 bg-slate-950/30 py-1 border-r border-b border-slate-700/50"
          >
            {day}
          </div>
        ))}
        ...
        {blanks.map((_, i) => (
          <div
            key={`blank-${i}`}
            className="bg-slate-950/30 border-r border-b border-slate-700/30 min-h-[120px]"
          ></div>
        ))}
        {/* Days */}
        {days.map((date) => (
...
export const CalendarView: React.FC<Props> = ({
  ...
  if (loads.length === 0) {
    return (
      <div className="flex h-full items-center justify-center bg-slate-900 rounded-xl border border-slate-700">
        <EmptyState
          icon={<CalendarIcon className="w-16 h-16" />}
          title="No loads scheduled"
          description="Add loads to see them on the calendar."
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full gap-4 relative">
          ...
          <div className="mb-4 bg-slate-900/50 p-2 rounded border border-slate-700">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  if (manualDateInput)
                    handleJumpToDate(new Date(manualDateInput));
                }}
      ...
      <div className="flex-1 flex flex-col bg-slate-900 h-full rounded-xl border border-slate-700 overflow-hidden shadow-2xl relative">
        ...
        <div
          ref={containerRef}
          className="flex-1 overflow-y-auto overflow-x-hidden relative bg-slate-900 scroll-smooth"
        >
          ...
          <div className="py-12 text-center text-slate-500 text-sm border-t border-slate-800 mt-4">
            ...
            <p className="text-xs mt-1">
              Use the Date Picker (top left) to jump to a different year.
            </p>
          </div>
        </div>
      </div>
