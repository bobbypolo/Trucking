...
import {
    MapPin, Truck, Navigation, Clock, Info,
    Search, Layers, Filter, Maximize2,
    Wifi, WifiOff, AlertCircle, Map as MapIcon,
    ShieldCheck, Calendar, ChevronRight, ChevronLeft
} from 'lucide-react';
...
export const GlobalMapView: React.FC<Props> = ({ loads, users, incidents = [], onViewLoad, onSelectIncident }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [leftPanelCollapsed, setLeftPanelCollapsed] = useState(false);
    const [rightPanelCollapsed, setRightPanelCollapsed] = useState(false);

    // Derived Mock GPS Data (In a real app, this would be from a live telemetry API)
    const activeVehicles = useMemo(() => {
        const drivers = users.filter(u => u.role === 'driver');
        return drivers.map((driver, idx) => {
            const activeLoad = loads.find(l => l.driverId === driver.id && l.status === LOAD_STATUS.Active);
            const isOnline = Math.random() > 0.1; // 90% online rate for mock
            return {
                driver,
                activeLoad,
                isOnline,
                ...
                coords: {
                    x: 15 + (idx * 12) % 70, // Spread across "map"
                    y: 20 + (idx * 15) % 65
                },
                heading: Math.floor(Math.random() * 360),
                speed: isOnline ? Math.floor(55 + Math.random() * 15) : 0,
                hasIncident: incidents.some(inc => inc.loadId === activeLoad?.id && inc.status !== 'Closed')
            };
        });
        ...
        <div className="h-full flex flex-col bg-[#020617] text-slate-100 font-inter overflow-hidden relative">
            ...
            <div className="absolute inset-0 z-0">
                ...
                <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 1000 600">
                    <path d="M50,150 Q250,50 450,200 T850,100" fill="none" stroke="#3b82f6" strokeWidth="1" />
                    <path d="M0,450 Q200,350 400,500 T900,400" fill="none" stroke="#3b82f6" strokeWidth="1" />
                    <path d="M300,0 Q500,300 300,600" fill="none" stroke="#3b82f6" strokeWidth="1" />
                </svg>
                ...
                {filteredVehicles.map((vehicle, idx) => (
                    <div
                        ...
                        style={{ left: `${vehicle.coords.x}%`, top: `${vehicle.coords.y}%`, transform: 'translate(-50%, -50%)' }}
                        onClick={() => {
                            if (vehicle.hasIncident) {
                                const inc = incidents.find(i => i.loadId === vehicle.activeLoad?.id && i.status !== 'Closed');
                    ...
                    >
                        ...
                        <div className={`relative w-6 h-6 rounded-lg border flex items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-transform group-hover:scale-125 ${vehicle.hasIncident ? 'bg-red-950 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)] animate-pulse' : vehicle.isOnline ? 'bg-slate-900 border-blue-500/50' : 'bg-slate-950 border-slate-800'}`}>
                            {vehicle.activeLoad ? (
                                <Navigation className={`w-3.5 h-3.5 ${vehicle.hasIncident ? 'text-red-400' : vehicle.isOnline ? 'text-blue-400' : 'text-slate-600'}`} style={{ transform: `rotate(${vehicle.heading}deg)` }} />
                            ) : (
                                <Truck className="w-3.5 h-3.5 text-slate-500" />
                            )}

                            {/* Ping Indicator Dot */}
                            <div className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full border border-slate-900 ${vehicle.hasIncident ? 'bg-red-500 animate-ping' : vehicle.isOnline ? 'bg-green-500 shadow-[0_0_5px_#22c55e]' : 'bg-red-500'}`} />
                        </div>
                        ...
                        <div className="absolute top-8 left-1/2 -translate-x-1/2 w-56 bg-[#0a0f1e] border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] p-4 opacity-0 group-hover:opacity-100 transition-all pointer-events-none translate-y-2 group-hover:translate-y-0">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <h4 className="text-[10px] font-black text-white uppercase tracking-tight">{vehicle.driver.name}</h4>
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        {vehicle.isOnline ? <Wifi className="w-2.5 h-2.5 text-green-500" /> : <WifiOff className="w-2.5 h-2.5 text-red-500" />}
                                        <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">{vehicle.isOnline ? 'Active' : 'Offline'} • {vehicle.speed} MPH</span>
                                    </div>
                                </div>
                            ...
                            {vehicle.activeLoad && (
                                <div className="bg-slate-950/50 border border-white/5 rounded-xl p-2.5 space-y-2">
                                    ...
                                    <div className="text-[9px] font-bold text-slate-300">
                                        {vehicle.activeLoad.pickup.city} → {vehicle.activeLoad.dropoff.city}
                                    </div>
                                </div>
                            )}

                            <div className="mt-3 flex items-center justify-between text-[7px] font-black text-slate-600 uppercase tracking-widest">
                                <span>LP: {new Date(vehicle.lastPing).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            ...
            <div className={`absolute top-8 left-8 z-10 space-y-4 w-80 transition-all duration-500 transform ${leftPanelCollapsed ? '-translate-x-[110%] opacity-0' : 'translate-x-0 opacity-100'}`}>
                ...
                <div className="bg-[#0a0f1e]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">Fleet Status</span>
                        <Filter className="w-3.5 h-3.5 text-slate-500" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-slate-950/50 p-3 rounded-xl border border-white/5">
                            <div className="text-[8px] font-black text-blue-500 uppercase tracking-widest">En Route</div>
                            <div className="text-lg font-black text-white mt-0.5">{activeVehicles.filter(v => v.activeLoad).length}</div>
                        </div>
                        <div className="bg-slate-950/50 p-3 rounded-xl border border-white/5">
                            <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Available</div>
                            <div className="text-lg font-black text-white mt-0.5">{activeVehicles.filter(v => !v.activeLoad).length}</div>
                        </div>
                    </div>
                </div>
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex gap-4">
                <div className="bg-yellow-500 border border-yellow-400/50 text-black px-6 py-3 rounded-2xl flex items-center gap-3 shadow-[0_0_30px_rgba(234,179,8,0.3)] animate-pulse cursor-pointer">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">3 Units Off-Route Warning</span>
                </div>
