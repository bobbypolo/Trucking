...
import {
    Search, Plus, Building2, Phone, Mail, Edit2, Trash2,
    MoreVertical, Users, Check, X, Info, ShieldCheck, ArrowRight,
    Scan, MapPin, Globe, ShieldAlert, AlertTriangle
} from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { Broker, Contract, ApprovedChassis } from '../types';
import { getBrokers, saveBroker, getContracts, saveContract } from '../services/brokerService';
...
export const BrokerManager: React.FC<Props> = ({ onUpdate, onSave, onAddLoad }) => {
    ...
    const handleAddChassis = () => {
        ...
        const newChassis: ApprovedChassis = {
            ...
            prefixes: prefixInput.split(',').map(p => p.trim()).filter(p => p.length > 0)
        };
    ...
    const handleRemoveChassis = (id: string) => {
        setEditingBroker(prev => {
            ...
            return {
                ...prev,
                approvedChassis: (prev.approvedChassis || []).filter(c => c.id !== id)
            };
        });
    };
        ...
        <div className="h-full flex flex-col bg-[#020617] text-slate-100 font-inter">
            ...
            <div className="bg-[#0a0f1e]/80 backdrop-blur-md border-b border-white/5 px-8 py-6 sticky top-0 z-10">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-white tracking-widest uppercase">Partner Network</h1>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Commercial Entity & Managed Registry</p>
                        </div>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-6 no-scrollbar grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                {filteredBrokers.map(broker => (
                    <div key={broker.id} className="bg-[#0a0f1e]/40 border border-white/5 rounded-2xl p-6 transition-all hover:bg-[#0a0f1e]/60 hover:border-blue-500/30 group relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="flex gap-2">
                                ...
                                <button
                                    onClick={() => { setEditingBroker(broker); setShowForm(true); }}
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <h3 className="text-sm font-black text-white uppercase tracking-tight line-clamp-1">{broker.name}</h3>
                                        <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter ${broker.clientType === 'Broker' ? 'bg-blue-600/10 text-blue-500 border border-blue-500/20' : 'bg-purple-600/10 text-purple-500 border border-purple-500/20'}`}>
                                            {broker.clientType}
                                        </span>
                                    </div>
                                    <div className="flex gap-3 text-[10px] font-bold text-slate-500">
                                        <span className="flex items-center gap-1"><Info className="w-3 h-3" /> MC: {broker.mcNumber || 'N/A'}</span>
                                        <span className="flex items-center gap-1 text-blue-500/80"><ShieldCheck className="w-3 h-3" /> Score: {broker.safetyScore || 85}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 text-[10px] font-medium text-slate-400">
                                        ...
                                        <span className="truncate">{broker.email || 'CONTACT PENDING'}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-[10px] font-medium text-slate-400">
                                ...
                                <div className="flex items-end justify-end">
                                    <div className="flex -space-x-2">
                                        {[1, 2].map(i => (
                                            <div key={i} className="w-6 h-6 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-[8px] font-bold text-slate-600">
                                                U{i}
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {broker.approvedChassis && broker.approvedChassis.length > 0 && (
                                <div className="pt-3 flex flex-wrap gap-2">
                                    {broker.approvedChassis.slice(0, 2).map((c, idx) => (
                                        <span key={idx} className="bg-slate-950 text-[8px] font-black px-2 py-1 rounded-md border border-white/5 uppercase text-slate-400">
                                            {c.provider} {c.type}
                                        </span>
                                    ))}
                                    {broker.approvedChassis.length > 2 && (
                                        <span className="text-[8px] font-black text-blue-500 self-center">+{broker.approvedChassis.length - 2} MORE</span>
                                    )}
                                </div>
                            )}
                        </div>
                ...
                <div className="fixed inset-0 z-[100] bg-[#020617]/90 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-[#0a0f1e] rounded-[2.5rem] border border-white/10 w-full max-w-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden flex flex-col max-h-[92vh]">
                        ...
                        <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-[#0d1428]/50">
                            <div>
                                <h2 className="text-2xl font-black text-white tracking-tight">{editingBroker.name ? 'Edit Client' : 'Add New Client'}</h2>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-1">Registry Entity Configuration</p>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Client Type</label>
                                    <div className="relative">
                                        <select
                                            ...
                                            onChange={e => setEditingBroker({ ...editingBroker, clientType: e.target.value as any })}
                                        >
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div
                                        onClick={() => setEditingBroker({ ...editingBroker, isShared: !editingBroker.isShared })}
                                    ...
                                    >
                                        <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${editingBroker.isShared ? 'bg-blue-600 border-blue-500' : 'bg-slate-900 border-slate-700 group-hover:border-slate-500'}`}>
                                            {editingBroker.isShared && <Check className="w-3.5 h-3.5 text-white" />}
                                        </div>
                                        <span className="text-sm font-bold text-slate-400 group-hover:text-slate-200">{editingBroker.isShared ? 'Shared (Team)' : 'Private (Me)'}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Name Input */}
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Legal Entity Name *</label>
                                <input
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">MC Number</label>
                                    <input
                                        ...
                                        onChange={e => setEditingBroker({ ...editingBroker, mcNumber: e.target.value })}
                                    />
                                </div>
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">DOT Number</label>
                                    <input
                                        className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-4 text-sm text-white font-bold placeholder:text-slate-800 outline-none focus:border-blue-500 transition-all"
                                        placeholder="e.g. 1928374"
                                        value={editingBroker.dotNumber}
                                        onChange={e => setEditingBroker({ ...editingBroker, dotNumber: e.target.value })}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Central Phone</label>
                                    <input
                                        className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-4 text-sm text-white font-bold placeholder:text-slate-800 outline-none focus:border-blue-500 transition-all"
                            ...
                            {/* Address */}
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Mailing Address / City / State</label>
                                <input
                                    className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-4 text-sm text-white font-bold placeholder:text-slate-800 outline-none focus:border-blue-500 transition-all"
                            ...
                            <div className="space-y-6 pt-6 border-t border-white/5">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <input
                                                ...
                                                onChange={e => setChassisForm({ ...chassisForm, provider: e.target.value })}
                                            />
                                        </div>
                                        <div className="relative">
                                            <select
                                                ...
                                                onChange={e => setChassisForm({ ...chassisForm, type: e.target.value })}
                                            >
                                ...
                                <div className="space-y-3">
                                    {(!editingBroker.approvedChassis || editingBroker.approvedChassis.length === 0) ? (
                                        <p className="text-[10px] text-slate-600 font-bold italic text-center p-4 bg-slate-950/30 rounded-xl border border-dashed border-white/5">No chassis rules defined for this entity.</p>
                                    ) : (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            {editingBroker.approvedChassis.map(rule => (
                                                <div key={rule.id} className="bg-slate-950 p-4 rounded-xl border border-white/5 flex justify-between items-start group">
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{rule.provider}</span>
                                                            <span className="text-[10px] font-bold text-slate-500">•</span>
                                                            <span className="text-[10px] font-bold text-slate-200">{rule.type}</span>
                                                        </div>
                                                        <div className="flex flex-wrap gap-1 mt-2">
                                                            {rule.prefixes.map((px, i) => (
                                                                <span key={i} className="bg-blue-600/10 text-blue-400 text-[8px] font-black px-1.5 py-0.5 rounded border border-blue-600/20">{px}</span>
                                                            ))}
                                                        </div>
                                                    </div>
                                                    <button
