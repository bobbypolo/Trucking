...
export const getRawBrokers = (): Broker[] => {
  try {
    ...
    if (brokers.length === 0) {
      ...
      brokers = [
        { id: 'B-101', name: 'Global Logistics Partner', mcNumber: 'MC22910', email: 'ops@globallogistics.com', phone: '312-555-0101', address: 'Chicago, IL', clientType: 'Broker', creditScore: 92, approvedChassis: ['STD40', 'TRI20'] },
        { id: 'B-102', name: 'Intermodal Direct', mcNumber: 'MC44892', email: 'bookings@idirect.com', phone: '713-555-0102', address: 'Houston, TX', clientType: 'Forwarder', creditScore: 88, approvedChassis: ['STD40'] },
        { id: 'B-103', name: 'CH Robinson (Regional)', mcNumber: 'MC66120', email: 'dispatch@chrobinson.com', phone: '952-555-0103', address: 'Eden Prairie, MN', clientType: 'Broker', creditScore: 95, approvedChassis: ['STD40', 'TRI20', 'TRI40'] },
        { id: 'B-104', name: 'JB Hunt Intermodal', mcNumber: 'MC88301', email: 'support@jbhunt.com', phone: '479-555-0104', address: 'Lowell, AR', clientType: 'Carrier', creditScore: 98, approvedChassis: ['ALL'] },
        { id: 'B-105', name: 'Coyote Logistics', mcNumber: 'MC11200', email: 'loadboard@coyote.com', phone: '877-626-9683', address: 'Chicago, IL', clientType: 'Broker', creditScore: 91, approvedChassis: ['STD40', 'TRI20'] }
      ];
...
export const getBrokers = async (companyId?: string): Promise<Broker[]> => {
  try {
    const url = companyId ? `${API_URL}/clients/${companyId}` : `${API_URL}/clients`;
    ...
    if (response.ok) {
      ...
      return data.map((b: any) => ({
        ...b,
        approvedChassis: typeof b.chassis_requirements === 'string' ? JSON.parse(b.chassis_requirements) : (b.chassis_requirements || []),
        clientType: b.type
  ...
  try {
    ...
    if (response.ok) {
      ...
      return data.map((c: any) => ({
        ...c,
        equipmentPreferences: typeof c.equipment_preferences === 'string' ? JSON.parse(c.equipment_preferences) : (c.equipment_preferences || {})
      }));
    }
  } catch (e) {
...
export const checkSafetyScore = (mcNumber: string): number => {
  ...
  const seed = mcNumber.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
