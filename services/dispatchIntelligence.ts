...
export const DispatchIntelligence = {
     ...
     */
    getBestMatches: async (load: LoadData, drivers: User[]): Promise<DispatchOpportunity[]> => {
        ...
        const opportunities: DispatchOpportunity[] = drivers
            ...
            .map(driver => {
                ...
                const dist = Math.sqrt(Math.pow(lat - disabledLat, 2) + Math.pow(lng - disabledLng, 2)) * 69; // Quick approx
                ...
                let recommendation: 'STRONG_MATCH' | 'CONSIDER' | 'DO_NOT_ASSIGN' = 'CONSIDER';
                ...
                return {
                    ...
                    estimatedArrival: new Date(Date.now() + (dist / 55) * 3600000).toISOString(),
    ...
    predictExceptionRisk: (load: LoadData | undefined | null): { risk: 'LOW' | 'MEDIUM' | 'HIGH', reason?: string } => {
        if (!load) return { risk: 'LOW' };
        if (!load.pickupDate) return { risk: 'MEDIUM', reason: 'Missing pickup schedule' };
        ...
        if (load.status === LOAD_STATUS.Active) {
            if (hoursUntil < estTransitHours) {
                return { risk: 'HIGH', reason: 'Behind schedule: ETA exceeds appointment' };
            }
            if (hoursUntil < estTransitHours + 2) {
                return { risk: 'MEDIUM', reason: 'Narrow buffer for scheduled arrival' };
            }
        }

        if (load.status === LOAD_STATUS.Planned && hoursUntil < 4 && !load.driverId) {
    ...
    analyzeProfitability: (load: LoadData): ProfitabilityAnalysis => {
        ...
        // Mocked cost estimation
        const estMiles = load.miles || 500;
        const fuelCost = estMiles * 0.60; // $0.60 cpm
        const driverPay = load.driverPay || (estMiles * 0.55); // $0.55 cpm
        const tolls = 45.00;
        ...
        return {
            ...
            recommendation: marginPercentage > 20 ? 'ACCEPT' : marginPercentage > 10 ? 'RE-NEGOTIATE' : 'DECLINE'
        };
    },
    ...
    calculateDynamicBid: (load: LoadData, capacityFactor: number): DynamicBid => {
        ...
        const baseRate = miles * 2.50;

        // Market adjustment (simulated AI prediction)
        const marketVolatility = 0.15; // 15% volatility
        const marketAverage = baseRate * (1 + (Math.random() * marketVolatility - marketVolatility / 2));
    ...
    getCapacityForecast: (loads: LoadData[]): CapacityForecast[] => {
        ...
        const forecast = regions.map(region => {
            const arrivingTrucks = loads.filter(l => {
                ...
                return dropDate && dropDate > now && dropDate < future72h && getRegion(l.dropoff.city) === region;
            }).length;
            ...
            return {
                ...
                emptyTrucks,
                predictedVolume,
                riskLevel: (emptyTrucks < 5 && predictedVolume > 80 ? 'HIGH' : emptyTrucks < 10 ? 'MEDIUM' : 'LOW') as 'LOW' | 'MEDIUM' | 'HIGH'
            };
        });
    ...
    auditSettlement: (load: LoadData): SettlementAudit => {
        ...
        const deductions = (load.expenses || []).reduce((acc, exp) => acc + (exp.amount || 0), 0);
        ...
        if (load.status !== LOAD_STATUS.Delivered && load.status !== LOAD_STATUS.Completed) {
    ...
    getDriverPerformance: (driverId: string, loads: LoadData[]): DriverPerformance => {
        ...
        if (totalLoads === 0) {
            return { driverId, onTimeDeliveryRate: 100, reliabilityScore: 100, safetyIncidents: 0, fuelEfficiency: 6.5, totalMiles: 0, rank: 'STANDARD' };
        }

        const onTimeLoads = driverLoads.filter(l => !l.issues?.some(i => i.category === 'Dispatch' && i.description.toLowerCase().includes('late'))).length;
        const reliabilityIssues = driverLoads.filter(l => l.issues?.some(i => i.category === 'Dispatch' || i.category === 'Safety')).length;
        const safetyIncidents = driverLoads.reduce((acc, l) => acc + (l.issues?.filter(i => i.category === 'Incident' || i.category === 'Safety').length || 0), 0);
        ...
        if (onTimeRate > 98 && reliabilityScore > 95 && safetyIncidents === 0) rank = 'ELITE';
        ...
        return {
            ...
            reliabilityScore,
            safetyIncidents,
            fuelEfficiency: 6.8 + (Math.random() * 0.5 - 0.25), // Mocked based on sensor data
            totalMiles,
            rank
        };
    },
    ...
    reconcileIFTATax: (loads: LoadData[]): IFTAAudit => {
        ...
        const totalFuel = loads.reduce((acc, l) => acc + (l.fuelPurchases?.reduce((fAcc, f) => fAcc + (f.gallons || 0), 0) || 0), 0);

        const alerts: string[] = [];
        if (totalMiles > 0 && totalFuel === 0) alerts.push('Miles recorded with zero fuel purchases');

        const avgMpg = totalFuel > 0 ? totalMiles / totalFuel : 6.5;
        if (avgMpg > 9) alerts.push('Improbably high MPG detected (Check fuel reporting)');
        if (avgMpg < 4) alerts.push('Critically low MPG - potential fuel theft or leakage');

        // Mocked financial reconciliation
        const netTaxDue = totalMiles * 0.045 - (totalFuel * 0.15); // Simple mock formula
        ...
        return {
            ...
            netTaxDue,
            status: netTaxDue > 100 ? 'UNDERPAID' : netTaxDue < -100 ? 'OVERPAID' : 'BALANCED',
