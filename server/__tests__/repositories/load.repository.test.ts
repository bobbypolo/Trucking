...
describe("R-P2-01: Load Repository", () => {
  ...
  describe("AC2: create — transactional load + stops creation", () => {
    ...
    it("sets sequence_order based on stop array index", async () => {
      ...
      // Stop insert params: (id, load_id, type, facility_name, city, state, date, appointment_time, completed, sequence_order, latitude, longitude)
      const stopParams1 = mockQuery.mock.calls[1][1];
      const stopParams2 = mockQuery.mock.calls[2][1];

      // sequence_order is at index 9 (0-based), followed by latitude and longitude
      const seqIdx = 9;
      expect(stopParams1[seqIdx]).toBe(0);
      expect(stopParams2[seqIdx]).toBe(1);
    });
  });
