...
export const loadRepository = {
    ...
    async create(
        ...
        try {
            ...
            for (let i = 0; i < stops.length; i++) {
                ...
                await connection.query(
                    `INSERT INTO load_legs (id, load_id, type, facility_name, city, state, date, appointment_time, completed, sequence_order)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        stopId,
                        loadId,
    ...
    async update(
        ...
        const allowedFields = [
            'customer_id', 'driver_id', 'dispatcher_id', 'load_number',
            'status', 'carrier_rate', 'driver_pay', 'pickup_date',
            'freight_type', 'commodity', 'weight', 'container_number',
            'container_size', 'chassis_number', 'chassis_provider',
            'bol_number', 'notification_emails', 'contract_id',
            'gps_history', 'pod_urls', 'customer_user_id',
        ];
