...
describe("CommandCenterView", () => {
  ...
  it("renders bottom alert strip with operational status", async () => {
    render(<CommandCenterView {...defaultProps} />);
    // operationalStreamStatus is "Nominal" when no open incidents
    expect(
      screen.getByText(/Global Operational Stream: Nominal/),
    ).toBeInTheDocument();
    // unitsLabel is computed: defaultProps has 1 driver (driver-1) with an in_transit load
    // so connectedCount=1, totalUnits=1 → "1/1 Units Online"
    expect(screen.getByText(/1\/1 Units Online/)).toBeInTheDocument();
    // lastSyncTime starts as null → syncLabel = "Awaiting Data"
    expect(screen.getByText(/Last Sync: Awaiting Data/)).toBeInTheDocument();
  });
