 ...
 *   - Demo server running: npm run demo:serve (in another terminal)
 *
 * For a permanent URL, set up a named Cloudflare Tunnel instead:
...
async function main() {
  ...
  if (!serverUp) {
    process.stderr.write(
      `[tunnel] Demo server not responding at ${LOCAL_URL}/api/health\n` +
