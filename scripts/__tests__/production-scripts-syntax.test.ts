...
describe('provision-production.sh', () => {
  ...
  it('R-P2-08: production provisioning grants secretAccessor role', () => {
    // Tests R-P2-08
    const matches = provisionContent.match(/secretmanager\.secretAccessor|secretAccessor/g);
    expect(matches).not.toBeNull();
    expect(matches!.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('R-P2-10: production provisioning grants serviceAccountUser role to deployer', () => {
    ...
    const matches = provisionContent.match(/serviceAccountUser|iam\.serviceAccountUser/g);
    expect(matches).not.toBeNull();
    expect(matches!.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('script enables automated backups (backup flag present)', () => {
    ...
    const hasBackup = provisionContent.includes('--backup') || provisionContent.includes('enable-bin-log');
  ...
  it('script targets correct GCP project ID', () => {
    const matches = provisionContent.match(/gen-lang-client-0535844903/g);
    expect(matches).not.toBeNull();
...
describe('deploy-production.sh', () => {
  ...
  it('R-P2-14: deploy script uses --min-instances=1 (no cold starts in production)', () => {
    ...
    const hasMinInstances1 =
      deployContent.includes('--min-instances=1') || deployContent.includes('--min-instances 1');
  ...
  it('deploy script prints deployed revision name', () => {
    const hasRevision = deployContent.includes('REVISION_NAME') || deployContent.includes('revision');
...
describe('.env.production', () => {
  ...
  it('R-P2-18: .env.production sets DB_SOCKET_PATH for production Cloud SQL', () => {
    expect(envContent).toContain('DB_SOCKET_PATH=/cloudsql/gen-lang-client-0535844903:us-central1:loadpilot-prod');
  });
  ...
  it('R-P2-18: .env.production contains "Do NOT commit" warning', () => {
    const hasWarning = envContent.includes('Do NOT commit') || envContent.includes('do not commit');
