...
def test_r_p2_15_handle_callback_encrypts_tokens():
    ...
    result = _run_qb_tests()
    assert result.returncode == 0, (
        f"QuickBooks callback tests failed:\n{result.stdout}\n{result.stderr}"
    )
...
def test_r_p2_16_get_client_decrypts_refreshes():
    ...
    result = _run_qb_tests()
    assert result.returncode == 0, (
        f"QuickBooks getClient tests failed:\n{result.stdout}\n{result.stderr}"
    )
...
def test_r_p2_17_sync_invoice():
    ...
    result = _run_qb_tests()
    assert result.returncode == 0, (
        f"QuickBooks invoice sync tests failed:\n{result.stdout}\n{result.stderr}"
    )
...
def test_r_p2_18_encrypt_decrypt_roundtrip():
    ...
    assert result.returncode == 0, (
        f"QuickBooks crypto roundtrip tests failed:\n{result.stdout}\n{result.stderr}"
    )
...
def test_r_p2_19_missing_env_graceful():
    ...
    result = _run_qb_tests()
    assert result.returncode == 0, (
        f"QuickBooks graceful fallback tests failed:\n{result.stdout}\n{result.stderr}"
    )
