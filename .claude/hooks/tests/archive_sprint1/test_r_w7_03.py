...
def test_r_w7_03a_cert_expiry_checker_exports_check_expiring():
    ...
    assert (
        "export async function checkExpiring" in src
        or "export function checkExpiring" in src
    ...
    assert (
        "INTERVAL" in src or "daysAhead" in src.lower() or "days_ahead" in src.lower()
    ), "Query must accept days-ahead parameter"


def test_r_w7_03b_creates_notification_jobs():
    """R-W7-03b: Service generates alert records (notification jobs) for expiring certs"""
    src = (SERVER / "services" / "cert-expiry-checker.ts").read_text(encoding="utf-8")
    ...
    assert "createExpiryAlerts" in src, "Must export createExpiryAlerts function"
    assert "notification_jobs" in src, "Must insert into notification_jobs table"
    assert "ExpiryAlertResult" in src, "Must define ExpiryAlertResult interface"
...
def test_r_w7_03c_safety_route_exposes_expiring_certs():
    ...
    assert "checkExpiring" in src, "safety.ts must import and call checkExpiring"
...
def test_r_w7_vpc_802_unit_tests_exist():
    ...
    assert "createExpiryAlerts" in content, (
        "Tests must import and test createExpiryAlerts"
    )
    # Verify R-marker
    ...
    assert "expiring-certs" in route_content, (
        "Route test must test expiring-certs endpoint"
    )
