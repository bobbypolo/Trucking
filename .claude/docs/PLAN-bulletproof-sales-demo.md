Bulletproof Sales Demo — CORRECTED Plan (live functions only)
 ...
 handler.
 2. Production-code touches are limited to THREE surgical files, all documented in the File Inventory:
   - App.tsx — nav filter wrapper + Reset Demo button injection, env-flag-gated (Phase 6, <30 added lines).
   - server/index.ts — conditional mount of the /api/demo router, env-flag-gated (Phase 6, 2 added lines).
   - server/repositories/document.repository.ts — ≤10-line column-alias mapping that aliases
     original_filename → filename and document_type → type on every returned document row (Phase 2).
     This is the SINGLE additional production-code exception beyond the two demo-shell surfaces,
     and it is required because the canonical documents UI at LoadDetailView.tsx:1125-1135 reads
     `doc.filename` and `doc.type` while the raw repository rows only expose `original_filename`
     and `document_type`. Without this mapping the buyer sees "undefined undefined" labels on
     every document card in the hero load. The mapping lives at the repository layer only;
     document.service.ts, LoadDetailView.tsx, and every other file in hard rule 1 stay untouched.
 3. Two acceptable categories of NEW code only:
 ...
 4. No backwards-compat shims, no helper modules wrapped around production functions, no "demo branch" inside
 any existing handler.
 5. Production tenant behavior is byte-for-byte identical before and after this sprint. The only way to enter
 demo mode is via env vars that are not set in production. The document.repository.ts alias mapping is
 additive (the original columns remain present) so production callers that read `original_filename` /
 `document_type` are unaffected.
 ...
 **Phase Type**: `foundation`
 ...
 ```bash
 bash -c "cd server && npx vitest run __tests__/scripts/seed-sales-demo.test.ts __tests__/scripts/reset-sales-demo.test.ts __tests__/middleware/sales-demo-tier-audit.test.ts __tests__/docs/sales-demo-seed-contract.test.ts"
 ```
 ...
 **Phase Type**: `module`
 ...
 ```bash
 bash -c "cd server && npx vitest run __tests__/scripts/seed-sales-demo-loads.test.ts __tests__/integration/sales-demo-load-readback.test.ts __tests__/integration/sales-demo-document-download.test.ts __tests__/repositories/document.repository.mapping.test.ts && cd .. && E2E_SERVER_RUNNING=1 npx playwright test e2e/sales-demo/01-document-automation.spec.ts"
 ```
 ...
 **Phase Type**: `integration`
 ...
 ```bash
 bash -c "cd server && npx vitest run __tests__/scripts/seed-sales-demo-ifta.test.ts __tests__/integration/sales-demo-ifta-summary.test.ts && cd .. && E2E_SERVER_RUNNING=1 npx playwright test e2e/sales-demo/02-ifta-walkthrough.spec.ts"
 ```
 ...
 **Phase Type**: `integration`
 ...
 ```bash
 bash -c "cd server && npx vitest run __tests__/scripts/seed-sales-demo-parties.test.ts __tests__/integration/sales-demo-network-portal.test.ts && cd .. && E2E_SERVER_RUNNING=1 npx playwright test e2e/sales-demo/03-network-portal-walkthrough.spec.ts"
 ```
 ...
 **Phase Type**: `module`
 ...
 ```bash
 npx vitest run __tests__/components/SafetyView.sales-demo.test.tsx __tests__/components/GlobalMapViewEnhanced.sales-demo.test.tsx __tests__/components/GoogleMapsAPITester.sales-demo.test.tsx && cd server && npx vitest run __tests__/docs/sales-demo-seed-contract.test.ts
 ```

 ---
## Phase 6 — Demo Shell: Nav Allowlist + Reset Button + Reset Endpoint (primary production-code touch)
 ...
 **Phase Type**: `integration`
 ...
 This phase owns the two env-flag-gated demo-shell touches to production source code: App.tsx (nav filter +
 reset button) and server/index.ts (one-line route mount). The only OTHER production-code touch in the
 sprint is the ≤10-line column-alias mapping in server/repositories/document.repository.ts owned by
 Phase 2 (see Phase 2 R-P2-12). No other production files are modified across the entire sprint; see
 the File Inventory table below for the full exhaustive list (3 SaaS source files + 2 build/docs files).
 ...
 ```bash
 npx vitest run __tests__/services/demoNavConfig.test.ts __tests__/App.demo-nav.test.tsx && cd server && npx vitest run __tests__/routes/demo.test.ts __tests__/docs/sales-demo-runbook.test.ts
 ```
 ...
 **Phase Type**: `e2e`
 ...
 ```bash
 npx vitest run __tests__/scripts/demo-certify.test.ts && cd server && npx vitest run __tests__/docs/sales-demo-e2e-readme.test.ts __tests__/docs/release-evidence.test.ts && cd .. && npm run demo:certify:sales
 ```

 ---
## File Inventory (extend vs create) — final, verified

 Extended (existing files modified) — exactly 5 files in the entire repository touched; of those,
 3 are SaaS source (App.tsx, server/index.ts, server/repositories/document.repository.ts) and 2 are
 build/docs (package.json, docs/release/evidence.md):

 ┌────────────────────────────────────────┬──────────┬─────────────────────────────────────────────────────┐
 │                   File                 │ Phase(s) │                     Diff scope                      │
 ├────────────────────────────────────────┼──────────┼─────────────────────────────────────────────────────┤
 │ package.json (root)                    │ 1, 7     │ 3 added script lines (demo:reset:sales,             │
 │                                        │          │ demo:seed:sales, demo:certify:sales)                │
 ├────────────────────────────────────────┼──────────┼─────────────────────────────────────────────────────┤
 │ App.tsx                                │ 6        │ < 30 added lines: 1 import + nav filter wrapper +   │
 │                                        │          │ reset button injection (all if (isDemoNavMode())    │
 │                                        │          │ gated)                                              │
 ├────────────────────────────────────────┼──────────┼─────────────────────────────────────────────────────┤
 │ server/index.ts                        │ 6        │ 2 added lines: import + conditional                 │
 │                                        │          │ ALLOW_DEMO_RESET mount                              │
 ├────────────────────────────────────────┼──────────┼─────────────────────────────────────────────────────┤
 │ server/repositories/document.          │ 2        │ ≤ 10 added lines, 0 removed lines: column alias     │
 │ repository.ts                          │          │ mapping (original_filename → filename,              │
 │                                        │          │ document_type → type). Single additional            │
 │                                        │          │ production-code exception; required so the docs    │
 │                                        │          │ panel on the hero load renders readable labels     │
 │                                        │          │ instead of "undefined undefined". See R-P2-12.      │
 ├────────────────────────────────────────┼──────────┼─────────────────────────────────────────────────────┤
 ...
 **Continuity rule for sales**: step 2's broker, step 3's trip, and step 4's CRM record MUST all
 ...
 This section is the authoritative dependency graph for sprint execution. It is derived from the
 Ralph prd.json specification block above and mirrored here so the audit tooling can find it as a
 dedicated top-level section.

 **Internal dependencies (story-to-story):**

 | Story | Phase | Type         | parallelGroup | dependsOn                            |
 | ----- | ----- | ------------ | ------------- | ------------------------------------ |
 | S-001 | 1     | foundation   | 0             | (none — dispatched first)            |
 | S-002 | 2     | module       | 1             | [S-001]                              |
 | S-003 | 3     | integration  | 1             | [S-001, S-002]                       |
 | S-004 | 4     | integration  | 1             | [S-001]                              |
 | S-005 | 5     | module       | 1             | [S-001]                              |
 | S-006 | 6     | integration  | 1             | [S-001]                              |
 | S-007 | 7     | e2e          | 2             | [S-002, S-003, S-004, S-006]         |

 **Dependency graph (ralph dispatch order):**

 ```
 S-001 (group 0, no deps — dispatched first)
   ├→ S-002  (group 1)
   │   └→ S-003  (group 1, waits for S-002)
   ├→ S-004  (group 1)
   ├→ S-005  (group 1)
   └→ S-006  (group 1)
         └→ S-007  (group 2, waits for S-002 + S-003 + S-004 + S-006)
 ```

 With `parallel_dispatch_enabled: true` and `parallel_batch_size: 3` (verified in
 `.claude/workflow.json`), ralph dispatches S-001 first, then a parallel batch from {S-002, S-004,
 S-005, S-006} (3 at a time), then S-003 after S-002 completes, then S-007 last.

 **External dependencies (environment, infrastructure, libraries):**

 | Dependency                        | Required for           | How it is provisioned                          |
 | --------------------------------- | ---------------------- | ---------------------------------------------- |
 | Firebase Auth user for admin      | Phases 1, 2, 3, 4, 6, 7| Manual — created in Firebase Console, UID     |
 |                                   |                        | pasted into `.env.local` as                    |
 |                                   |                        | `SALES_DEMO_ADMIN_FIREBASE_UID`                |
 | Firebase Auth user for driver     | Phases 1, 6 runbook    | Manual — same as admin, UID into               |
 |                                   |                        | `SALES_DEMO_DRIVER_FIREBASE_UID`               |
 | MySQL instance with live schema   | Phases 1-4             | Existing project DB (shared dev)               |
 | `.env.local` (single env contract)| Phases 1, 6 reset, 7   | Copy `.env.example.sales-demo` → fill 7 values |
 | Playwright + chromium             | Phases 2, 3, 4, 7      | Existing `playwright.config.ts` at `./e2e`     |
 | `FIREBASE_WEB_API_KEY` env var    | Phase 7 smoke + hero   | Existing project-wide auth fixture variable    |
 | `ts-node` (`--transpile-only`)    | Phases 1, 7            | Existing dependency                            |
 | Migration 038 (column rename)     | Phase 1 reset DELETEs  | Already applied at HEAD — verified by Agent C  |
 | Migration 011 (fuel_ledger)       | Phase 3 fuel seeds     | Already applied at HEAD — verified             |
 | Migration 037 (parties FK)        | Phase 4 cascade        | Already applied at HEAD — verified             |

 **Build / CI dependencies:**

 - `npm run demo:reset:sales` → depends on Phase 1 + Phase 2/3/4 seed helpers, run in order.
 - `npm run demo:certify:sales` → depends on reset + Phase 7 certify harness + Phase 2/3/4 specs.
 - The full pre-existing regression suite (`cd server && npx vitest run` + root vitest) is a
   stretch gate, not a blocking gate — see Verification section below.
